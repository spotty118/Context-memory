...
def sync_model_catalog() -> Dict[str, Any]:
    ...
    try:
        ...
        or_models = openrouter.fetch_all_models()
...
def _update_model(existing_model: ModelCatalog, or_model: Dict[str, Any]) -> bool:
    ...
    updates = {
        ...
        "provider": or_model.get("provider", "").split("/")[0] if or_model.get("provider") else None,
        "description": or_model.get("description"),
        "context_length": or_model.get("context_length"),
        "input_cost": _parse_cost(or_model.get("pricing", {}).get("prompt")),
        "output_cost": _parse_cost(or_model.get("pricing", {}).get("completion")),
        "supports_streaming": or_model.get("supports_streaming", True),
        "supports_functions": or_model.get("supports_functions", False),
    }
    ...
    if existing_model.status == "deprecated" and or_model.get("id"):
...
def _create_model(db: Session, or_model: Dict[str, Any]) -> ModelCatalog:
    ...
    model = ModelCatalog(
        id=or_model["id"],
        name=or_model.get("name", or_model["id"]),
        provider=or_model.get("provider", "").split("/")[0] if or_model.get("provider") else "unknown",
        description=or_model.get("description"),
        context_length=or_model.get("context_length"),
        input_cost=_parse_cost(or_model.get("pricing", {}).get("prompt")),
        output_cost=_parse_cost(or_model.get("pricing", {}).get("completion")),
...
def _parse_cost(cost_str: Optional[str]) -> Optional[float]:
    ...
    Args:
        cost_str: Cost string like "0.03" or "$0.03"
    
    Returns:
        Cost as float or None
    """
    if not cost_str:
        return None
    ...
    try:
        ...
        clean_cost = str(cost_str).replace("$", "").strip()
