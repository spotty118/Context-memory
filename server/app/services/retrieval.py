...
class ContextRetriever:
    ...
    async def _get_semantic_items(self, thread_id: str, db: Session) -> List[Dict[str, Any]]:
        """Get all semantic items for a thread."""
        query = db.query(SemanticItem).filter(SemanticItem.thread_id == thread_id)
        items = query.all()
    ...
    async def _get_episodic_items(self, thread_id: str, db: Session) -> List[Dict[str, Any]]:
        """Get all episodic items for a thread."""
        query = db.query(EpisodicItem).filter(EpisodicItem.thread_id == thread_id)
        items = query.all()
    ...
    async def _get_usage_stats(self, item_ids: List[str], db: Session) -> Dict[str, Dict[str, Any]]:
        """Get usage statistics for items."""
        if not item_ids:
            return {}
        
        query = db.query(UsageStats).filter(UsageStats.item_id.in_(item_ids))
        stats = query.all()
