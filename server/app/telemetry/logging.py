...
def add_correlation_id(logger: Any, method_name: str, event_dict: EventDict) -> EventDict:
    ...
    # Try to get correlation ID from FastAPI request context
    try:
        from fastapi import Request
        from starlette.context import context
        
        # Try to get request from context
        if "request" in context:
            request = context["request"]
            if hasattr(request, "state") and hasattr(request.state, "correlation_id"):
                event_dict["correlation_id"] = request.state.correlation_id
                event_dict["request_id"] = request.state.correlation_id
    except ImportError:
